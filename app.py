...
def corregi_query_sql(query_originale: str, errore: str, domanda: str, schema: str) -> str:
    """Chiede a Gemini di correggere una query Oracle che ha dato errore, mostrandole l'errore esatto."""
    prompt = f"""Questa query Oracle SQL ha dato errore in fase di esecuzione.

RICHIESTA ORIGINALE DELL'UTENTE: "{domanda}"
...
ERRORE RESTITUITO DA ORACLE:
{errore}

Correggi la query tenendo conto dell'errore. Ricorda in particolare che:
- Le colonne che contengono date sono spesso salvate come testo (CLOB/VARCHAR), non come DATE:
  per confrontarle o fare calcoli, usa prima TO_DATE(colonna, 'YYYY-MM-DD').
...
spiegazioni, senza punto e virgola finale."""
    ...
    query = _llm_invoke_cached(prompt, MODEL_SIMPLE).strip()
    query = re.sub(r"^```(sql)?|```$", "", query, flags=re.MULTILINE).strip().rstrip(";")
    return query


def esegui_query_con_autocorrezione(query: str, domanda: str, schema: str):
    """Esegue la query; se Oracle restituisce un errore, chiede a Gemini di correggerla e riprova UNA sola volta."""
    try:
        colonne, righe = esegui_query_sicura_sola_lettura(query)
        return colonne, righe, query
    except Exception as e:
        messaggio_errore = str(e)
        if "ORA-" not in messaggio_errore:
            raise
        query_corretta = corregi_query_sql(query, messaggio_errore, domanda, schema)
        colonne, righe = esegui_query_sicura_sola_lettura(query_corretta)
        return colonne, righe, query_corretta


def genera_riepilogo_naturale(domanda: str, colonne, righe) -> str:
    """Genera una breve interpretazione in linguaggio naturale dei risultati di una query, con il dato principale in evidenza."""
    if not righe:
        return "Non ho trovato risultati per questa richiesta."

    campione = righe[:15]
    tabella_testo = " | ".join(colonne) + "\n" + "\n".join(
        " | ".join(str(v) for v in riga) for riga in campione
    )

    prompt = f"""L'utente ha chiesto: "{domanda}"

Questi sono i risultati ottenuti da una query sul database (colonne e prime righe):
{tabella_testo}

Scrivi una risposta breve (2-4 frasi) in italiano che interpreti questi dati e risponda
direttamente alla domanda dell'utente, evidenziando il valore/risultato più rilevante in
**grassetto**. Non ripetere l'intera tabella: quella verrà mostrata separatamente. Sii diretto
e concreto, con i numeri esatti che vedi nei dati."""
...
if submit and user_msg.strip():
    ...
    with st.spinner("Elaborazione della richiesta in corso..."):
        try:
            ...
            schema_corrente = _schema_per_prompt(tabelle)
            ...
            if intento == "altro":
                ...
                colonne, righe, query_finale = esegui_query_con_autocorrezione(query, user_msg, schema_corrente)
                riepilogo = genera_riepilogo_naturale(user_msg, colonne, righe)
